...
const WORKFLOW_ACTIONS = [
  { key: 'APPROVE',   label: 'Approve',   color: 'text-status-pass-fg' },
  { key: 'REJECT',    label: 'Reject',    color: 'text-status-fail-fg' },
  { key: 'SEND_BACK', label: 'Send back', color: 'text-status-warn-fg' },
  { key: 'DELEGATE',  label: 'Delegate',  color: 'text-status-tag-fg' },
  { key: 'ESCALATE',  label: 'Escalate',  color: 'text-status-warn-fg' },
  { key: 'REASSIGN',  label: 'Reassign',  color: 'text-status-info-fg' },
  { key: 'COMMENT',   label: 'Comment',   color: 'text-text-muted' },
  { key: 'WITHDRAW',  label: 'Withdraw',  color: 'text-status-fail-fg' },
]
      ...
      footer={
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {hasOverride && (
              <button onClick={handleClear} className="text-xs text-text-muted hover:text-status-fail-fg transition-colors">
    ...
    >
      <div className="flex gap-5 min-h-80">
        ...
        <div className="w-36 shrink-0 space-y-0.5">
          ...
          {!entityType && (
            <div className="mt-3 px-3 py-2 rounded-card bg-status-warn-bg border border-status-warn-bd">
              <p className="text-[10px] text-status-warn-fg">Set entityType on the blueprint to see real fields</p>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {section === 'tabs' && (
            <div className="space-y-4">
              ...
              {/* Mode toggle */}
              <div className="flex items-center gap-3 p-3 rounded-card border border-border bg-surface-overlay">
                  ...
                  return (
                    <button key={tab.key} onClick={() => toggleTab(tab.key)}
                      ...
                      <div className={cn('w-5 h-5 rounded flex items-center justify-center shrink-0',
                        visible ? 'bg-status-pass-bg' : 'bg-surface-raised border border-border')}>
                        {visible ? <Eye size={11} className="text-status-pass-fg" /> : <EyeOff size={11} className="text-text-muted" />}
                      </div>
                  ...
                  <div className="space-y-1">
                    {(sec.fields || [])
                      ...
                      .map(f => {
                        ...
                        return (
                          <div key={f.key} className="flex items-center gap-3 px-3 py-2 rounded-card border border-border hover:bg-surface-overlay/50 transition-colors">
                            <div className="flex-1 min-w-0">
                              <span className="text-xs text-text-primary">{f.label}</span>
                              <span className="text-[10px] font-mono text-text-muted ml-2">{f.key}</span>
                            </div>
                            {/* Mode selector */}
                            <div className="flex items-center rounded-ctl border border-border overflow-hidden shrink-0">
                              {[
                                { m: 'default',  label: 'Default', color: 'text-text-muted' },
                                { m: 'editable', label: 'Editable', color: 'text-status-pass-fg' },
                                { m: 'readonly', label: 'Read-only', color: 'text-status-warn-fg' },
                                { m: 'hidden',   label: 'Hidden', color: 'text-status-fail-fg' },
                              ].map(opt => (
                ...
                {WORKFLOW_ACTIONS.map(action => {
                  ...
                  return (
                    <button key={action.key} onClick={() => toggleAction(action.key)}
                      className={cn(
                        'w-full flex items-center gap-3 px-3 py-2.5 rounded-card border text-left transition-colors',
                        available ? 'border-status-pass-bd bg-status-pass-bg' : 'border-border opacity-50'
                      )}>
                      <div className={cn('w-5 h-5 rounded flex items-center justify-center shrink-0',
                        available ? 'bg-status-pass-bg' : 'bg-surface-raised border border-border')}>
                        {available && <CheckCircle2 size={11} className="text-status-pass-fg" />}
                      </div>
          ...
          {section === 'preview' && (
            <div className="space-y-3">
              ...
              <pre className="text-[11px] font-mono text-text-secondary bg-surface-overlay border border-border rounded-card p-4 overflow-x-auto leading-relaxed">
              ...
              {hasOverride && (
                <div className="space-y-1.5 text-xs">
                  ...
                  {state.editableFields?.length > 0 && (
                    <div className="flex gap-2"><span className="text-text-muted w-28">Editable only</span><span className="text-status-pass-fg font-mono">{state.editableFields.join(', ')}</span></div>
                  )}
                  {state.readOnlyFields?.length > 0 && (
                    <div className="flex gap-2"><span className="text-text-muted w-28">Read-only</span><span className="text-status-warn-fg font-mono">{state.readOnlyFields.join(', ')}</span></div>
                  )}
                  {state.hiddenFields?.length > 0 && (
                    <div className="flex gap-2"><span className="text-text-muted w-28">Hidden fields</span><span className="text-status-fail-fg font-mono">{state.hiddenFields.join(', ')}</span></div>
                  )}
                  {state.availableActions?.length > 0 && (
                    <div className="flex gap-2"><span className="text-text-muted w-28">Actions only</span><span className="text-status-info-fg font-mono">{state.availableActions.join(', ')}</span></div>
                  )}
                </div>
              )}
            </div>
